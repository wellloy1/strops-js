...
function getIndexesOfAllSubstringsFromAtoBExclusive(str, a, b) {
  ...
  let lastIndex = 0;
  while (true) {
    const a_index = str.indexOf(a, lastIndex) + a.length;
    const b_index = str.indexOf(b, lastIndex + a.length);
...
function getIndexesOfAllSubstringsFromAtoB(str, a, b) {
  ...
  let lastIndex = 0;
  while (true) {
    const a_index = str.indexOf(a, lastIndex);
    const b_index = str.indexOf(b, lastIndex + a.length) + b.length;
...
function getAllSubstringsFromAtoB(str, a, b) {
  ...
  for (const startIndex in entrySlices) {
    arr.push(
      str.substring(startIndex - a.length, entrySlices[startIndex] + b.length)
    );
  }
  return arr;
}
